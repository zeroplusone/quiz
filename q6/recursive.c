...
void reverseSub(node** head, sub** mnSub)
{
    node* first;
    node* rest;

    assert((*head)!= NULL && "input subarray is null");
...
node* reverseBetween(node* head, int m, int n)
{
    ...
    if(m==1) {
        /* m is the first node */
        reverseSub(&head,&mnSub);
        head = mnSub->subHead;
    } else {
        reverseSub(&(pre->next),&mnSub);
